...
//#ifdef _DEBUG
    #define CARET_TIMER_INTERVAL 500 //100000   // Background poll (not the main driver; event-driven detection handles most updates)
//#else
...
#define DETECT_REASONS \
    X(DETECT_REASON_KEY_DOWN,          0) /* lParam = virtual key code */ \
    X(DETECT_REASON_MOUSE_BUTTON_DOWN, 1) /* unused - mouse-down only sets g_bMouseButtonDown flag */ \
    ...
    X(DETECT_REASON_MOUSE_BUTTON_UP,   5) /* mouse button released */ \
    X(DETECT_REASON_KEY_UP,            6) /* lParam = virtual key code */
